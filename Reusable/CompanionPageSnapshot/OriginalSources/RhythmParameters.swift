...
struct RhythmPreset {
    ...
    private static let fallback = RhythmPreset(
        name: "安稳入睡",
        description: "低刺激、稳定的陪伴节奏",
        ageTier: .newborn,
        scenario: .nightSleep,
        baseBpm: 60,
        noiseRatios: (0.10, 0.30, 0.60),
        noiseLevel: 0.10,
        melodyVolume: 0.03,
        melodyComplexity: 0.10,
        highFreqAttenuation: 0.80,
        dynamicRange: 0.40,
        segmentRatios: (0.25, 0.35, 0.40)
    )
...
enum RhythmParameterMapper {
    ...
    static func extractInputs(
        ...
        let todayEnd = calendar.date(byAdding: .day, value: 1, to: todayStart)
            ?? todayStart.addingTimeInterval(24 * 60 * 60)

...
struct ContentView: View {
    ...
    @State private var activeQuickRecordEditTarget: QuickRecordEditTarget?
    @State private var activeVoiceRecord: VoiceRecordRoute?
    ...
    @State private var isGrowthDiaryPresented = false
    @State private var bottomDockLayoutEpoch = UUID()
    @State private var feedbackDeferralID = UUID()
    @State private var showFamilySharingFromSyncBanner = false
    @State private var activeMorningBBBrief: DailyReportSnapshot?
    @State private var pendingBBBriefReward: BBBuckTransaction?
    @State private var bbBriefRewardPresentationTask: Task<Void, Never>?
    @AppStorage("bbbrief_last_morning_prompt_report_key_v1")
    private var lastMorningPromptReportKey = ""
    ...
    private var tabViewContent: some View {
        tabViewShell
            .onAppear {
                updateCelebrationPresentationDeferral(shouldDeferCelebrationPresentation)
                reconcileAndPresentBBBriefIfNeeded()
            }
            .onDisappear {
                if !shouldDeferCelebrationPresentation {
                    updateCelebrationPresentationDeferral(false)
                }
            }
            .onChange(of: shouldDeferCelebrationPresentation) { _, shouldDefer in
                updateCelebrationPresentationDeferral(shouldDefer)
                if !shouldDefer {
                    reconcileAndPresentBBBriefIfNeeded()
                }
            }
            ...
            .onChange(of: activeQuickRecordEditTarget?.id) { _, _ in
                dismissQuickAddMenu()
            }
            ...
            .fullScreenCover(item: $activeVoiceRecord) { _ in
                VoiceRecordView()
            }
            ...
            .sheet(isPresented: $showFamilySharingFromSyncBanner) {
                NavigationStack {
                    FamilySharingView()
                }
            }
            .fullScreenCover(item: $activeMorningBBBrief) { snapshot in
                BBBriefPage(
                    snapshot: snapshot,
                    ...
                    onDismiss: dismissMorningBBBrief
                )
            }
            .task(id: scenePhase) {
                ...
                while !Task.isCancelled {
                    reconcileAndPresentBBBriefIfNeeded()
                    let wait = max(bbBriefStore.nextAvailabilityDate().timeIntervalSinceNow, 1)
                    do {
    ...
    private var shouldDeferCelebrationPresentation: Bool {
        activeRecordSheet != nil
            || activeQuickRecordKind != nil
            || activeQuickRecordEditTarget != nil
            || activeVoiceRecord != nil
            || subjectiveStatePrompt != nil
            || showQuickAddMenu
            || activeMorningBBBrief != nil
    }

    private var feedbackDeferralReason: String {
        "content-input-flow:\(feedbackDeferralID.uuidString.lowercased())"
    }
    ...
    private var canPresentMorningBBBrief: Bool {
        scenePhase == .active
            && activeMorningBBBrief == nil
            && activeRecordSheet == nil
            && activeQuickRecordKind == nil
            ...
            && subjectiveStatePrompt == nil
            && !showQuickAddMenu
            && !showFamilySharingFromSyncBanner
    }

    private func reconcileAndPresentBBBriefIfNeeded(now: Date = Date()) {
        guard hasCompletedOnboarding else { return }
        let latest = bbBriefStore.reconcile(
            now: now,
            feedingSessions: feedingStore.allSessions,
            careRecords: activityStore.exportCareRecords(),
            subjectiveCheckIns: SubjectiveStateStore.shared.checkIns,
            babyAgeMonths: profileStore.currentProfile.ageMonths,
            legacyReports: recruitmentStore.reports
        )
        guard canPresentMorningBBBrief,
              let latest,
              latest.reportKey != lastMorningPromptReportKey,
              !bbBriefStore.wasOpened(latest.reportKey) else { return }

        lastMorningPromptReportKey = latest.reportKey
        activeMorningBBBrief = latest
    }

    private func dismissMorningBBBrief() {
        activeMorningBBBrief = nil
        bbBriefRewardPresentationTask?.cancel()
        bbBriefRewardPresentationTask = Task { @MainActor in
            try? await Task.sleep(for: .milliseconds(280))
            guard !Task.isCancelled,
                  activeMorningBBBrief == nil,
                  let transaction = pendingBBBriefReward else { return }
            pendingBBBriefReward = nil
            BBBriefRewardFeedback.present(transaction)
            lastPresentedReportRewardTransactionID = transaction.id
            bbBriefRewardPresentationTask = nil
        }
    }
    ...
    private var tabViewShell: some View {
            ...
            rootTabs
                ...
                .safeAreaInset(edge: .bottom, spacing: 0) {
                    if shouldShowBottomDock {
                        Color.clear
                            .frame(height: BottomDockLayout.contentClearanceHeight)
                            .accessibilityHidden(true)
                    }
                }

            // Keep the three responsibilities separate:
            // rootTabs is the scrolling content, the fade protects that content,
            // and the interactive dock always renders above the fade.
            if shouldShowBottomDock {
                bottomDockOverlay
        ...
        // The root dock must stay attached to the window's container safe area.
        // After a long lock/unlock cycle iOS can briefly retain the keyboard safe
        // area even though no keyboard is visible, which otherwise lifts the
        // whole dock by roughly one keyboard height. Ignore only the keyboard
        // region so the Home Indicator inset still applies.
        .ignoresSafeArea(.keyboard, edges: .bottom)
        .safeAreaInset(edge: .top, spacing: 0) {
            if AppVariant.isFamilySyncEnabled, familyCloudStore.syncStatus.shouldShowBanner {
                FamilySyncBanner(
                    status: familyCloudStore.syncStatus,
                    onRetry: { familyCloudStore.retryNow() },
                    onViewDetails: { showFamilySharingFromSyncBanner = true }
                )
        ...
        .overlay {
            if let activeQuickRecordEditTarget {
                QuickRecordCardOverlay(
                    editTarget: activeQuickRecordEditTarget,
                    onDismiss: {
                        self.activeQuickRecordEditTarget = nil
                    },
                ...
                .transition(.opacity)
                .zIndex(21)
            } else if let activeQuickRecordKind {
                QuickRecordCardOverlay(
        ...
        .animation(.interactiveSpring(response: 0.28, dampingFraction: 0.86), value: activeQuickRecordEditTarget?.id)
    }
    ...
    private var rootTabs: some View {
        TabView(selection: $selectedTab) {
            Tab(value: RootTab.record) {
                NavigationStack {
                    RecordHomeView(
                        ...
                        onEditQuickRecord: { target in
                            activeQuickRecordEditTarget = target
                        }
                    )
                }
            ...
            Tab(value: RootTab.growth) {
                NavigationStack {
                    MyPageView(isGrowthDiaryPresented: $isGrowthDiaryPresented)
                }
    ...
    private func handleDeepLink(_ url: URL) {
        guard let deepLink = BabyBuddyDeepLink(url: url) else { return }

        switch deepLink {
        case .overview:
            openReadOnlyOverview()
        }
    }

    private func openReadOnlyOverview() {
        // A system-surface tap is an inspection request. Dismiss any stale
        // creation UI before selecting the overview so a widget/Live Activity
        // tap can never reopen a feeding, diaper, or sleep editor.
        activeRecordSheet = nil
        activeQuickRecordKind = nil
    ...
    private func openVoiceRecord() {
        dismissQuickAddMenu()
        activeRecordSheet = nil
        activeQuickRecordKind = nil
        activeQuickRecordEditTarget = nil
        activeVoiceRecord = VoiceRecordRoute()
    }
    ...
    private var shouldShowBottomDock: Bool {
        activeRecordSheet == nil && !isGrowthDiaryPresented
    }
...
struct RootPageTitleBar<Trailing: View>: View {
    let title: String
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 0) {
            Text(title)
                .font(BBBFont.font(size: RootPageTitleBarLayout.titleFontSize, weight: .bold))
        ...
        .frame(height: RootPageTitleBarLayout.rowHeight)
        .accessibilityElement(children: .contain)
    }
}

private enum BottomDockLayout {
    static let dockHeight: CGFloat = 64
    static let fadeLeadHeight: CGFloat = 64
    static let contentClearanceHeight = dockHeight
    static let visualProtectionHeight = dockHeight + fadeLeadHeight
}
...
private struct BottomDockVisualProtection: View {
    ...
    var body: some View {
        Color.clear
            .frame(height: BottomDockLayout.visualProtectionHeight)
            .frame(maxWidth: .infinity)
            // Shape-style backgrounds can extend through the container safe area
            // without changing the dock's own safe-area anchored position.
            .background(bottomFade, ignoresSafeAreaEdges: .bottom)
            .accessibilityHidden(true)
    }

    private var bottomFade: LinearGradient {
        let fadeColor = colorScheme == .dark ? DesignToken.scrim : DesignToken.surface
        let middleOpacity = colorScheme == .dark ? 0.72 : 0.54

        return LinearGradient(
            stops: [
                .init(color: fadeColor.opacity(0), location: 0),
                .init(color: fadeColor.opacity(middleOpacity), location: 0.46),
                .init(color: fadeColor.opacity(0.96), location: 0.72),
...
private struct BottomNavigationDock: View {
                ...
                if isSelected {
                    Circle()
                        ...
                        .overlay(
                            Circle()
                                .fill(
                                    DesignToken.primary.opacity(
                                        colorScheme == .dark ? 0.18 : 0.025
                                    )
                                )
                        )
                        .shadow(color: DesignToken.shadowColor.opacity(0.14), radius: 8, y: 3)
                        .matchedGeometryEffect(id: "bottomTabSelection", in: selectionBackground)
                }
    ...
    private func tabIcon(for tab: RootTab, isSelected: Bool) -> some View {
        if tab == .record {
            SimpleHomeGlyph(isFilled: true)
                .foregroundStyle(isSelected ? DesignToken.textPrimary : DesignToken.textPrimary.opacity(0.52))
    ...
    private func symbol(for tab: RootTab) -> String {
        switch tab {
        case .record:
            return "house.fill"
        case .companion:
...
private struct FloatingRecordAddButton: View {
    ...
    var body: some View {
        ZStack(alignment: .bottomTrailing) {
            ...
            Image(systemName: "plus")
                ...
                .contentShape(Circle())
                .gesture(mainButtonGesture)
                .accessibilityElement(children: .ignore)
                .accessibilityAddTraits(.isButton)
                .accessibilityLabel(isExpanded ? "收起记录菜单" : "打开记录菜单")
                .accessibilityHint("长按打开语音记录")
                .accessibilityAction {
                    toggleQuickAddMenu()
                }
                .accessibilityAction(named: "语音记录") {
                    openVoiceRecord()
                }
        }
    ...
    private var mainButtonGesture: some Gesture {
        LongPressGesture(minimumDuration: 0.45, maximumDistance: 22)
    ...
    private func toggleQuickAddMenu() {
...
struct QuickRecordCardOverlay: View {
    ...
    init(
        voiceDraft: VoiceRecordDraft,
        onDismiss: @escaping () -> Void,
        onCompletedRecord: @escaping (SubjectiveStatePromptContext) -> Void = { _ in }
    ) {
        let initialState = Self.initialState(for: voiceDraft)
        self.init(
            initialState: initialState,
            targetCycleID: nil,
            recordDate: initialState.recordedAt,
    ...
    var body: some View {
        ...
        .onReceive(Timer.publish(every: 1, on: .main, in: .common).autoconnect()) { date in
            ...
            case .sleep where sleepMode == .justAsleep && sleepDraftStore.isRecording:
                sleepDraftStore.updateCurrentTime(date)
                sleepEndAt = date
                value = max(sleepDraftStore.elapsedSeconds / 60, 0)
            default:
                break
            }
        }
    ...
    private static func initialState(for draft: VoiceRecordDraft) -> QuickRecordInitialState {
        switch draft {
        case .feeding(let milkKind, let amount, let recordedAt):
            let kind: QuickRecordKind
            switch milkKind {
            case .formula: kind = .formulaBottle
            case .expressed: kind = .expressedBottle
            case .nursing: kind = .nursing
            case .solids: kind = .solids
            }
        ...
        case .solid(let food, let amount, _, let recordedAt):
            var state = QuickRecordInitialState(
                kind: .solids,
                value: clamped(Int(amount.rounded()), for: .solids),
                recordedAt: recordedAt
            )
            state.selectedSolidFoods = [food]
            return state

        case .diaper(let diaperKind, let recordedAt):
            var state = QuickRecordInitialState(kind: .diaper, value: 1, recordedAt: recordedAt)
            switch diaperKind {
            case .peeLow:
                state.diaperMode = .pee
                state.urineAmount = .low
            case .peeMedium:
                state.diaperMode = .pee
                state.urineAmount = .medium
            case .peeHigh:
                state.diaperMode = .pee
                state.urineAmount = .high
            case .poopHard:
                state.diaperMode = .poop
                state.poopTexture = .hard
            case .poopFormed:
                state.diaperMode = .poop
                state.poopTexture = .formed
            case .poopPaste:
                state.diaperMode = .poop
                state.poopTexture = .paste
            case .poopWatery:
                state.diaperMode = .poop
                state.poopTexture = .watery
            case .poopMucus:
                state.diaperMode = .poop
                state.poopTexture = .mucus
            }
            return state
        ...
        case .activity(let title, let durationMinutes, let recordedAt):
            var state = QuickRecordInitialState(
                kind: .activity,
                value: clamped(durationMinutes ?? QuickRecordKind.activity.defaultValue, for: .activity),
                recordedAt: recordedAt
            )
            state.selectedActivityItems = activityItems(from: title)
            return state

        case .sleep(let startAt, let endAt):
            let duration = SleepRecordFormatter.durationMinutes(start: startAt, end: endAt)
            var state = QuickRecordInitialState(
                kind: .sleep,
            ...
            state.sleepStartAt = startAt
            state.sleepEndAt = endAt
            return state

        case .growth(_, _, let recordedAt):
            // Growth drafts use their dedicated editor in VoiceRecordView.
        ...
        case .subjective(_, _, let recordedAt):
            // Subjective-state drafts use their dedicated editor in VoiceRecordView.
            return QuickRecordInitialState(
                kind: .activity,
                value: QuickRecordKind.activity.defaultValue,
                recordedAt: recordedAt
    ...
    private static func feedingInitialState(for session: FeedingSession) -> QuickRecordInitialState {
        let recordedAt = session.eventDate
    ...
    private static func careInitialState(for record: CareRecord) -> QuickRecordInitialState {
        ...
        case .activity:
            let duration = SleepRecordFormatter.durationMinutes(from: record.detail)
                ?? QuickRecordKind.activity.defaultValue
            var state = QuickRecordInitialState(
                kind: .activity,
                value: clamped(duration, for: .activity),
    ...
    private var quickRecordTimePicker: some View {
        Button {
            ...
            HStack(spacing: 6) {
                Image(systemName: isRecordingToday ? "clock" : "calendar.badge.clock")
                    .font(.system(size: 12, weight: .bold))
                Text(recordDateContextText)
            ...
            .frame(height: 34)
            .frame(width: isRecordingToday ? 136 : 166)
        ...
        .accessibilityValue(AppDateTimeFormat.dateTime(recordTimeReferenceDate))
        .disabled((selectedKind == .sleep && sleepMode == .justAsleep) || isLiveNursingTiming)
    }

    private var recordDateContextText: String {
        if isRecordingToday {
            return AppLocalization.format("今天 %@", AppDateTimeFormat.time(recordTimeReferenceDate))
        }
        return AppLocalization.format(
            "补录 %@ %@",
            recordedAtDayText,
            AppDateTimeFormat.time(recordTimeReferenceDate)
        )
    }
    ...
    private var recordTimeEditorText: String {
        AppDateTimeFormat.dateTime(recordTimeReferenceDate)
    }

    private func syncRecordTimePickerState() {
        let components = Calendar.current.dateComponents([.hour, .minute], from: recordTimeReferenceDate)
    ...
    private func applyRecordTimePickerState() {
        ...
        var components = Calendar.current.dateComponents([.year, .month, .day], from: recordTimeReferenceDate)
        ...
        let updatedRecordTime = min(candidate, Date())
        if selectedKind == .sleep {
            let duration = max(sleepEndAt.timeIntervalSince(sleepStartAt), 60)
            sleepStartAt = updatedRecordTime
            sleepEndAt = min(updatedRecordTime.addingTimeInterval(duration), Date())
            value = sleepDurationMinutes
        } else {
            recordedAt = updatedRecordTime
        }
    }

    /// Sleep records are stored at their start time, so the header and the
    /// "入睡" control intentionally share the same source of truth.
    private var recordTimeReferenceDate: Date {
        if selectedKind == .sleep {
            return sleepStartAt
        }
        if isLiveNursingTiming {
            return timerTick
        }
        return recordedAt
    }

    private var isLiveNursingTiming: Bool {
        selectedKind == .nursing && editTarget == nil && feedingDraftStore.breastTimingStartedAt != nil
    }
    ...
    private var isRecordingToday: Bool {
        Calendar.current.isDateInToday(recordTimeReferenceDate)
    }
    ...
    private var recordedAtDayText: String {
        AppDateTimeFormat.date(recordTimeReferenceDate)
    }
    ...
    private var nursingHero: some View {
        GeometryReader { proxy in
            ...
            // Keep the scales visually balanced around the hero art while
            // leaving enough breathing room for their end labels.
            let scaleCenterOffset = min(84, proxy.size.width * 0.255)
            ...
            ZStack {
                ...
                .position(x: centerX - scaleCenterOffset, y: centerY)
                ...
                Image("record_nursing_hero")
                    ...
                    .scaledToFit()
                    .opacity(0.70)
                    .frame(width: min(178, proxy.size.width * 0.54), height: 220)
    ...
    private var saveButton: some View {
        ...
        let saveIsEnabled = canSave && !isSaving
        ...
        return Button {
            ...
            HStack(spacing: 10) {
                Image(systemName: "checkmark.circle.fill")
                    ...
                    .foregroundStyle(
                        saveIsEnabled ? actionColor : actionColor.opacity(0.72),
                        saveIsEnabled ? DesignToken.onPrimary : actionColor.opacity(0.12)
                    )
                Text(saveButtonTitle)
                    .font(BBBFont.font(size: 19, weight: .heavy))
                    .foregroundStyle(saveIsEnabled ? DesignToken.onPrimary : actionColor.opacity(0.78))
            }
            .frame(maxWidth: .infinity)
            .frame(height: 58)
            .background(
            ...
            .overlay(
                Capsule()
                    .stroke(saveIsEnabled ? DesignToken.onPrimary.opacity(0.36) : actionColor.opacity(0.16), lineWidth: 1)
            )
            .shadow(color: actionColor.opacity(saveIsEnabled ? 0.20 : 0), radius: 14, y: 8)
        }
        .buttonStyle(ScaleButtonStyle())
    ...
    private var saveButtonTitle: String {
        ...
        if editTarget != nil {
            baseTitle = "保存修改".localized
        } else if selectedKind == .activity {
            baseTitle = canSave ? "保存活动".localized : "选择活动".localized
        } else if selectedKind != .sleep {
            baseTitle = "保存".localized
        } else if sleepMode == .justAsleep {
            baseTitle = sleepDraftStore.isRecording ? "保存睡眠".localized : "开始计时".localized
        } else {
            baseTitle = "保存睡眠".localized
        }

        guard editTarget == nil,
              !isRecordingToday,
              canSave,
              !(selectedKind == .sleep && sleepMode == .justAsleep && !sleepDraftStore.isRecording) else {
            return baseTitle
        }
        return AppLocalization.format("保存到 %@", recordedAtDayText)
    }
    ...
    private var canSave: Bool {
        ...
        case .sleep:
            if sleepMode == .justAsleep {
                guard let activeStart = sleepDraftStore.activeSleepStartAt else { return true }
            ...
            return sleepDurationMinutes >= QuickRecordKind.sleep.valueRange.lowerBound &&
                sleepDurationMinutes <= SleepRecordFormatter.maximumDurationMinutes
        default:
            return true
        }
    }
    ...
    private func save() {
        ...
        let submittedAt = isLiveNursingTiming ? min(timerTick, Date()) : effectiveRecordedAt
        AppHapticFeedback.impact(.medium)
    ...
    private func update(_ target: QuickRecordEditTarget) {
        ...
        switch target {
        case .feeding(let session):
            guard let entries = currentFeedingEntries(), !entries.isEmpty else {
                isSaving = false
                return
            }
            var updated = session.movingEventDate(to: effectiveRecordedAt)
        ...
        case .care(let record):
            ...
            case .activity:
                guard let title = activityRecordTitle else {
                    isSaving = false
                    return
                }
            ...
            case .sleep:
                guard let window = normalizedSleepWindow() else {
                    isSaving = false
                    return
                }
    ...
    private func saveSleep() {
        ...
        case .justAsleep:
            if let activeStart = sleepDraftStore.activeSleepStartAt {
                let end = Date()
                let duration = end.timeIntervalSince(activeStart)
                guard duration > 0,
                      duration <= TimeInterval(SleepRecordFormatter.maximumDurationMinutes * 60),
                      let record = activityStore.recordSleep(startTime: activeStart, endTime: end, note: "") else {
                    isSaving = false
                    return
                }
        ...
        case .justWoke, .smartFill, .manual:
            ...
            let durationMinutes = Int(window.end.timeIntervalSince(window.start) / 60)
            guard durationMinutes >= QuickRecordKind.sleep.valueRange.lowerBound,
                  durationMinutes <= SleepRecordFormatter.maximumDurationMinutes,
                  let record = activityStore.recordSleep(startTime: window.start, endTime: window.end, note: "") else {
                isSaving = false
                return
            }
            writtenRecord = record
...
struct FamilySyncBanner: View {
    let status: FamilySyncStatus
    let onRetry: () -> Void
    let onViewDetails: () -> Void

    private var isActionRequired: Bool {
        guard case .failed(let failure) = status.phase else { return false }
        switch failure {
        case .accountUnavailable, .permission, .quotaExceeded, .dataIntegrity:
            return true
        case .network, .serviceUnavailable, .unknown:
            return false
        }
    }
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(DesignToken.surface.opacity(0.96))
                .overlay(
                    RoundedRectangle(cornerRadius: 16, style: .continuous)
                        .stroke(tint.opacity(0.24), lineWidth: 1)
                )
        )
        // Keep the retry/view control as its own VoiceOver action while the
        // surrounding banner remains a single safe-area group.
        .accessibilityElement(children: .contain)
        .accessibilityIdentifier("family-sync.banner")
    }
}

enum BabyBuddyDeepLink: Equatable {
    case overview

    init?(url: URL) {
        guard url.scheme?.lowercased() == "babybuddy" else {
            return nil
        }
        ...
        case "record":
            // The former record/new/... URLs remain safe compatibility paths
            // for already-installed widgets and now resolve to the overview.
            guard path.isEmpty
                || (path.count == 2
                    && path[0] == "new"
                    && ["feeding", "diaper", "sleep"].contains(path[1])) else {
            return nil
        }
        default:
            return nil

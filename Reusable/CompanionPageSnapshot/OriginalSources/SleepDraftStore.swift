...
final class SleepDraftStore: ObservableObject {
    ...
    private func persistDraft() {
        ...
        let draft = SleepDraft(
            activeSleepStartAt: activeSleepStartAt,
            note: String(note.prefix(2_000)),
            updatedAt: Date()
        )
        guard let data = try? JSONEncoder().encode(draft) else { return }
        guard data.count <= BBBDataSafetyLimits.maxDraftDataBytes else { return }
    ...
    private func restoreDraft() {
        ...
        guard let data,
              data.count <= BBBDataSafetyLimits.maxDraftDataBytes,
              let draft = try? JSONDecoder().decode(SleepDraft.self, from: data) else { return }

        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            if records.isEmpty {
                ...
                ForEach(records) { record in
                    ...
                    .accessibilityAction(named: Text("删除记录")) {
                        pendingDeletion = record
                    }
...
struct GrowthMetricEntryView: View {
    ...
    private var valueCard: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if showsWHOHeightMeasurementGuidance {
                Label {
                    Text("2 岁前请记录卧位身长；若为站立测量，请先在测量值上加 0.7 cm。")
                        .fixedSize(horizontal: false, vertical: true)
                } icon: {
    ...
    private var assessment: WHOGrowthAssessment? {
        ...
        return GrowthReference.assessment(
            ...
            kind: kind,
            gender: profile.gender,
            ageDays: measurementAgeDays,
            value: enteredValue
        )
    }

    private var measurementAgeDays: Double {
        max(recordedAt.timeIntervalSince(profile.birthDate) / 86_400, 0)
    }

    private var showsWHOHeightMeasurementGuidance: Bool {
        kind == .height
            && growthStandard == .who2006
            && measurementAgeDays < 731
    }
    ...
    private var rulerHint: String {
        valueText.isEmpty
            ? AppLocalization.format(
                "尺子停在 %@ P50 参考起点；滑动后才会录入",
                growthStandard.shortTitle
            )
...
private struct WHOGrowthCurveView: View {
    ...
    private var maximumSupportedMonths: Double {
        GrowthReference.supportedAgeDays(for: standard).upperBound / 30.4375
    }
...
private struct GrowthRecordSwipeRow<Content: View>: View {
    ...
    var body: some View {
        ZStack(alignment: .trailing) {
            ...
            .buttonStyle(.plain)
            .background(DesignToken.error.opacity(0.88))
    ...
    private var rowOffset: CGFloat {
        let restingOffset = isDeleteVisible ? -actionWidth : 0
        return min(max(restingOffset + dragTranslation, -actionWidth), 0)
    }
    ...
    private var swipeGesture: some Gesture {
        DragGesture(minimumDistance: 12)
            ...
            .onEnded { value in
                ...
                withAnimation(.snappy(duration: 0.22)) {
                    isDeleteVisible = projectedOffset < -actionWidth * 0.42
                }
            }
    }
}

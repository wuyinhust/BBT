...
struct FeedingSheet: View {
    ...
    @State private var hasManuallyAdjustedRecordTime = false
    ...
    var body: some View {
        ...
        .onReceive(timer) { date in
            timerTick = date
            if !hasManualTimeSpan && !hasManuallyAdjustedRecordTime {
                recordTime = date
            }
        ...
        .onAppear {
            ...
            draftStore.updateCurrentTime(Date())
            recordTime = Date()
            hasManuallyAdjustedRecordTime = false
        }
        .onDisappear {
            if !didSave {
                persistDraft()
            ...
            DatePicker(
                ...
                selection: Binding(
                    ...
                    set: {
                        ...
                        hasManuallyAdjustedRecordTime = true
    ...
    private func loadSelectedPhoto() async {
        ...
        let jpeg = await Task.detached(priority: .userInitiated) {
            autoreleasepool {
                BBBImageDataImporter.downsampledJPEG(data: data, compressionQuality: 0.8)
            }
        }.value
    ...
    private func save() {
        guard !isSaving, recordTime <= Date() else { return }
        isSaving = true
        AppHapticFeedback.impact(.medium)
        commitActiveBreastElapsed()
    ...
    private func completePendingSave(confirmTimeSpan: Bool) {
        guard isSaving, let pendingSave else { return }

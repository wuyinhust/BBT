...
enum AppVariant {
    // Family sync is intentionally paused across every build variant. Keep the
    // implementation and stored CloudKit locator intact so the feature can be
    // restored only after the release/privacy work tracked in the unified Todo.
    static let isFamilySyncEnabled = false

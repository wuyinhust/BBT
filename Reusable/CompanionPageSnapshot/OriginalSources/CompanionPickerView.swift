...
struct CardCarouselIndicator: View {
    ...
    var body: some View {
        ...
        .frame(height: 22)
        .accessibilityLabel("\(accessibilityTitle) \(index + 1) / \(total)")
    }
}
...
struct CompanionDetailOverlay: View {
    ...
    var body: some View {
        ...
        return ZStack {
            AppModalBackdrop(bodyColor: backdrop.body, accent: backdrop.accent)
                .ignoresSafeArea()
            ...
            GeometryReader { proxy in
                ...
                VStack(spacing: 10) {
                    ...
                    if isUnlocked {
                        ...
                        BuddyInvitationPanel(
                            companion: displayCompanion,
                            onOpenTasks: onOpenTasks,
                            onRecruited: { recruitedCompanion = displayCompanion }
                        )
                            .onTapGesture {}
                    }
                }
            ...
            if let recruitedCompanion {
                BuddyRecruitmentCelebration(
                    companion: recruitedCompanion,
                    onDismiss: { self.recruitedCompanion = nil }
                )
                .transition(.opacity.combined(with: .scale(scale: 0.97)))
                .zIndex(30)
            }
        }
        .animation(.easeOut(duration: 0.22), value: recruitedCompanion?.id)
    ...
    private func buddyCard(
        ...
        return VStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                Text(isUnlocked ? companion.localizedIntro : "友情值 \(recruitmentStore.friendshipValue(for: companion.id))/\(companion.friendshipTarget)，使用 BB Bucks 直接邀请它成为伙伴。")
                    .font(BBBFont.font(size: 10, weight: .semibold))
                    .foregroundStyle(DesignToken.textSecondary)
                    .lineSpacing(2)
                    .fixedSize(horizontal: false, vertical: true)

                distributionRow(isUnlocked ? companion.localizedWorldDistribution : "每次邀请都会增加友情值，满值后立即解锁伙伴卡。")
            }
            .padding(.top, 13)
            .padding(.bottom, 2)
    ...
    private func carouselIndicator(for companion: BabyCompanion, tint: Color) -> some View {
        ...
        return CardCarouselIndicator(
            index: index,
            total: total,
            tint: tint,
            accessibilityTitle: "伙伴卡"
        )
    }
...
private struct BuddyInvitationPanel: View {
    ...
    var body: some View {
        VStack(spacing: 10) {
            HStack {
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text(AppLocalization.format(
                        "友情 %d/%d",
                        recruitmentStore.friendshipValue(for: companion.id),
            ...
            ProgressView(
                value: Double(recruitmentStore.friendshipValue(for: companion.id)),
                total: Double(max(companion.friendshipTarget, 1))
            )
            .tint(DesignToken.primary)

            HStack(spacing: 6) {
                amountChip("1", amount: 1, preset: "1")
                amountChip("3", amount: 3, preset: "3")
                amountChip("6", amount: 6, preset: "6")
                amountChip("全部".localized, amount: maxAmount, preset: "all")
                customChip
            }

            if isCustomAmount {
                HStack(spacing: 8) {
                    TextField("1-\(maxAmount)", text: $customAmountText)
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: 13, style: .continuous)
                                .fill(DesignToken.surfaceRaised.opacity(0.90))
                        )
                        .onChange(of: customAmountText) { _, value in
                            if let amount = Int(value) {
                                selectedAmount = min(max(amount, 1), max(maxAmount, 1))
                            }
                        }

                    Text(AppLocalization.format("友情 +%d", effectiveAmount))
                        .font(BBBFont.font(size: 10, weight: .heavy))
                        .foregroundStyle(DesignToken.primary)
                }
            }

            if let lastInvitedAmount {
                Text(AppLocalization.format(
                    "已使用 %d BB Bucks · 友情值 +%d",
                    lastInvitedAmount,
                    lastInvitedAmount
                ))
                    .font(BBBFont.font(size: 9.5, weight: .bold))
                    .foregroundStyle(DesignToken.success)
                    .frame(maxWidth: .infinity, alignment: .leading)
            }

            Button(action: inviteOrEarn) {
                HStack(spacing: 8) {
                    Image(systemName: recruitmentStore.bbBucks > 0 ? "paperplane.fill" : "checklist")
        ...
        .background(
            RoundedRectangle(cornerRadius: 22, style: .continuous)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 22, style: .continuous)
                        .stroke(DesignToken.glassStroke.opacity(0.78), lineWidth: 1)
                )
        )
        .padding(.horizontal, 18)
        .onAppear(perform: resetAmount)
        .onChange(of: companion.id) { _, _ in resetAmount() }
        .onChange(of: maxAmount) { _, _ in clampAmount() }
        .accessibilityIdentifier("buddy.invitation-panel")
    }

    private var maxAmount: Int {
        min(recruitmentStore.bbBucks, recruitmentStore.remainingFriendship(for: companion.id))
    }

    private var effectiveAmount: Int {
        min(max(selectedAmount, 1), max(maxAmount, 1))
    }

    private var primaryButtonTitle: String {
        guard recruitmentStore.bbBucks > 0 else { return "去完成任务赚 BB Bucks".localized }
        return AppLocalization.format("邀请 · %d BB Bucks", effectiveAmount)
    }

    private func amountChip(_ title: String, amount: Int, preset: String) -> some View {
        let resolved = min(max(amount, 1), max(maxAmount, 1))
        let isSelected = !isCustomAmount && selectedPreset == preset
        return Button {
            isCustomAmount = false
            selectedPreset = preset
            selectedAmount = resolved
        } label: {
            Text(title)
                .font(BBBFont.font(size: 10, weight: .heavy))
                .foregroundStyle(isSelected ? DesignToken.onPrimary : DesignToken.textSecondary)
                .frame(maxWidth: .infinity, minHeight: 34)
                .background(Capsule().fill(isSelected ? DesignToken.primary : DesignToken.surfaceSoft.opacity(0.82)))
        }
        .buttonStyle(.plain)
    ...
    private var customChip: some View {
        Button {
            isCustomAmount = true
            customAmountText = "\(effectiveAmount)"
        } label: {
            Image(systemName: "ellipsis")
                .font(.system(size: 11, weight: .heavy))
                .foregroundStyle(isCustomAmount ? DesignToken.onPrimary : DesignToken.textSecondary)
                .frame(maxWidth: .infinity, minHeight: 34)
                .background(Capsule().fill(isCustomAmount ? DesignToken.primary : DesignToken.surfaceSoft.opacity(0.82)))
        }
        .buttonStyle(.plain)
        .disabled(maxAmount == 0)
        .accessibilityLabel("自定义邀请数量")
    }

    private func inviteOrEarn() {
        guard recruitmentStore.bbBucks > 0 else {
            onOpenTasks()
            return
        }
        guard let result = recruitmentStore.invite(companionID: companion.id, amount: effectiveAmount) else { return }
        lastInvitedAmount = result.spentBucks
        if result.didRecruit {
    ...
    private func resetAmount() {
        selectedAmount = min(6, max(maxAmount, 1))
        selectedPreset = [1, 3, 6].contains(selectedAmount) ? "\(selectedAmount)" : "all"
        customAmountText = ""
        isCustomAmount = false
    }
    ...
    private func clampAmount() {
        if selectedPreset == "all" {
            selectedAmount = max(maxAmount, 1)
        } else {
            selectedAmount = min(max(selectedAmount, 1), max(maxAmount, 1))
        }
    }
}

private struct BuddyRecruitmentCelebration: View {
    let companion: BabyCompanion
    let onDismiss: () -> Void

    var body: some View {
        ZStack {
            DesignToken.scrim.opacity(0.34)
                .ignoresSafeArea()
            ...
            VStack(spacing: 14) {
                ...
                Text(AppLocalization.format("%@ 已成为伙伴", companion.localizedName))
                    .font(BBBFont.font(size: 22, weight: .heavy))
                    .foregroundStyle(DesignToken.textPrimary)
                Text("友情值已满，现在可以把它设为当前伙伴。")
                    .font(BBBFont.font(size: 12, weight: .semibold))
                    .foregroundStyle(DesignToken.textSecondary)
                    .multilineTextAlignment(.center)
                Button("太好啦", action: onDismiss)
                    .font(BBBFont.font(size: 13, weight: .heavy))
                    .foregroundStyle(DesignToken.onPrimary)
            ...
            .background(
                RoundedRectangle(cornerRadius: 28, style: .continuous)
                    ...
                    .shadow(color: DesignToken.shadowColor.opacity(0.24), radius: 24, y: 12)
            )
